...
$symbol = Select-String -Path $symbolsPath -Pattern '^([0-9a-f]+)\s+.*\.text\s+([0-9a-f]+)\s+TypedMachine\.runEntry$'
if (-not $symbol) { throw 'TypedMachine.runEntry symbol not found' }

...
void main() {
  ...
  test('await renames distinct inputs and preserves repeated inputs', () {
    final newInput = SSA('new_input');
    final newSecond = SSA('new_second');
    ...
    final distinct =
        Await(
              ...
              input,
              second,
            ).copyWith(readsFrom: {newInput, newSecond}, writesTo: output)
            as Await;
    expect(distinct.completer, newInput);
    expect(distinct.subject, newSecond);
    expect(distinct.result, output);
    final repeated =
        Await(
              result,
              input,
              input,
            ).copyWith(readsFrom: {newInput}, writesTo: output)
            as Await;
    expect(repeated.completer, newInput);
    expect(repeated.subject, newInput);
  });
  test('distinct await definitions remain distinct graph operations', () {
    expect(
      Await(result, input, second),
      isNot(Await(SSA('another_result'), input, second)),
    );
  });
}

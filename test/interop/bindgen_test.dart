...
void main() {
  ...
  check(const $optionalTextFn().call(runtime, null, [const $null()]) is $null);
  check((const $optionalTextFn().call(runtime, null, [null]) as $String).$value == 'default');
  check(($tripleFn.callRegisters(runtime, $int(1), $int(2), $int(3)) as $int).$value == 6);
  check(($Host.$total$registers(runtime, $int(2), $int(3), $int(4)) as $int).$value == 9);
  check(($Host.$many$registers(runtime, $int(1), $int(2), [$int(3), $int(4)]) as $int).$value == 10);
  check(($addDefaultFn.callRegisters(runtime, $int(2), null, 'unused stale register') as $int).$value == 7);
  $Host.$setCount$registers(runtime, $int(8), 'unused stale register', false);
  check(Host.count == 8);
  $Host.set$count$registers(runtime, $int(9), 'unused stale register', false);
  check(($Host.$count$registers(runtime, 'unused', 12, false) as $int).$value == 9);
  $assignFn.callRegisters(runtime, $int(10), false, 'unused');
  check(Host.count == 10);
  check($Host.$new$registers(runtime, $int(3), false, 'unused') is $Host);
  final overflow = <Object?>[$Function((runtime, target, args) => $int((args.single as $int).$value + 7)), $int(0)];
  $Host.$retain$registers(runtime, $int(1), $int(2), overflow);

...
class EvalAbstractClass extends EvalValueImpl<Type>
    implements DartInterface, EvalRuntimeType {
  EvalAbstractClass(this.declarations, this.generics, this.delegatedType,
      EvalScope lexicalScope,
      {String? sourceFile, this.superclassName, Type? realValue})
      : super(EvalType.typeType,
            sourceFile: sourceFile, fields: const {}, realValue: realValue) {
    this.lexicalScope = EvalObjectLexicalScope(lexicalScope);
    final inheritedScope = EvalObjectScope();
    addFields(_breakoutStaticDeclarations(
        declarations, this.lexicalScope, inheritedScope));
    inheritedScope.object = this;
    this.lexicalScope.object = this;
  }

  static Map<String, EvalField> _breakoutStaticDeclarations(
      List<DartDeclaration> declarations,
      EvalScope lexicalScope,
      EvalScope inheritedScope) {
    final entries = declarations
        .where((declaration) => declaration.isStatic)
        .expand((declaration) => declaration
            .declare(DeclarationContext.CLASS, lexicalScope, inheritedScope)
            .entries);
  ...
  List<DartDeclaration> getAllDeclarations(EvalScope lexicalScope) {
    ...
    final superclass = lexicalScope.lookup(superclassName!.refName)!.value
        as EvalAbstractClass;
    if (superclass is EvalBridgeAbstractClass) {
      return declarations;
    }

    return superclass
        .getAllDeclarations(lexicalScope)
        .followedBy(declarations)
        .toList();
  }
...
class EvalBridgeAbstractClass extends EvalAbstractClass
    with ValueInterop<Type> {
  EvalBridgeAbstractClass(List<DartDeclaration> declarations,
      EvalType delegatedType, EvalScope lexicalScope, Type realValue,
      {EvalGenericsList generics = EvalGenericsList.empty})
      : super(declarations, generics, delegatedType, lexicalScope,
            realValue: realValue,
            superclassName: delegatedType.supertypes[0],
            sourceFile: delegatedType.refSourceFile);
...
class EvalClass extends EvalAbstractClass implements EvalCallable {
  EvalClass(List<DartDeclaration> declarations, EvalType delegatedType,
      EvalScope lexicalScope, EvalGenericsList generics,
  ...
  @override
  EvalObject call(EvalScope _lex, EvalScope inheritedScope,
    ...
    final entries = _declarations
        ...
        .expand((declaration) => declaration
            .declare(
                DeclarationContext.CLASS_FIELD, objectLexicalScope, newScope)
            .entries);
...
class EvalBridgeClass<D extends BridgeRectifier> extends EvalBridgeAbstractClass
    implements EvalClass {
  EvalBridgeClass(List<DartDeclaration> declarations, EvalType delegatedType,
      EvalScope lexicalScope, Type realValue, this.instantiate,
      {EvalGenericsList generics = EvalGenericsList.empty})
      : super(declarations, delegatedType, lexicalScope, realValue,
            generics: generics);

  BridgeInstantiator<D> instantiate;

  BridgeRectifier<D> construct(String constructor, EvalScope lexicalScope,
    ...
    final me = instantiate(
            constructor,
            spl.positional.map((e) => e.value.evalReifyFull()).toList(),
            spl.named.map((k, v) => MapEntry(k, v.evalReifyFull())))
  ...
  @override
  BridgeRectifier<D> call(EvalScope lexicalScope, EvalScope inheritedScope,
      List<EvalType> generics, List<Parameter> args,
      {EvalValue? target}) {

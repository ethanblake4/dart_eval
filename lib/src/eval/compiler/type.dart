...
class TypeRef {
  ...
  factory TypeRef.fromBridgeTypeRef(
    ...
    if (spec != null) {
      ...
      return ctx.visibleTypes[lib]![spec.name]!
          .copyWith(specifiedTypeArgs: specifiedTypeArgs);
    }
    final ref = typeReference.ref;
    if (ref != null) {
      specifiedType ??=
          ctx.visibleTypes[ctx.library]![ctx.currentClass?.name2.stringValue];
      ...
      if (specifyingType != null) {
        ...
        if (syExtends != null &&
            syExtends.superclass.name.name == specifiedType.name) {
          final declaredType =
  ...
  factory TypeRef.lookupClassDeclaration(
    ...
    return ctx.visibleTypes[library]![cls.name.value()] ??
        (throw CompileError('Class ${cls.name.value()} not found'));
  }
  ...
  static TypeRef? lookupFieldType(
    ...
    if (dec.isBridge) {
      ...
      if ($extends == null) {
        ...
        final $super =
            ctx.visibleTypes[$class.file]![$extends.superclass.name.name]!;
        return TypeRef.lookupFieldType(
            ctx, $super.inheritTypeArgsFrom(ctx, $class), field);
      }
    }
  ...
  bool isAssignableTo(CompilerContext ctx, TypeRef slot,
    ...
    final typeChain = resolveTypeChain(ctx);
    for (final type in typeChain.allSupertypes) {
...
class AlwaysReturnType implements ReturnType {
  ...
  static AlwaysReturnType? fromInstanceMethodOrBuiltin(
    ...
    if (!$static &&
      ...
      return returnType.toAlwaysReturnType(ctx, type, argTypes, namedArgTypes,
          typeArgs: typeArgs);
    }

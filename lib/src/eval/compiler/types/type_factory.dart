...
final class TypeFactory {
  ...
  Map<String, TypeRef>? findMixinApplication(
    ...
    for (final mixinType in classLikeClauses(decl).$2) {
      ...
      final ref = _ctx.visibleTypes[declFile]?[mixinName];
      if (ref == null) continue;
      final mixinDecl =
          _ctx.topLevelDeclarationsMap[ref.file]?[ref.name]?.declaration;
      final mixinParams =
          switch (mixinDecl) {
      ...
      final applied = <String, TypeRef>{
        for (var i = 0; i < mixinParams.length; i++)
          mixinParams[i].name.lexeme:
              ...
              substitutedParamBound(declFile, mixinParams[i], substitutions),
      };
  ...
  TypeRef substitutedParamBound(
    ...
    return bound == null
        ? CoreTypes.dynamic.ref(_ctx)
        : fromAnnotation(file, bound).substituteTypeParameters(substitutions);
  }

  /// For [member] folded into [applier] from a mixin or mixin-class (possibly
  /// through a chain of mixin applications), seeds [CompilerContext.typeScopes]
  /// in the member's declaring library so its type parameters resolve to the
  /// applied arguments, expressed in [applier]'s own type parameters.
  void seedFoldedMemberTypeParams(
    ...
    _ctx.typeParameterScope(memberLibrary).addAll(applied);
  }
